...
def depthFirstSearch(problem: SearchProblem):
    ...
    while not frontier.isEmpty():
        ...
        # if not in reached appennd and if isGoal return its action
        if node not in reached:
            reached.append(node)
            if problem.isGoalState(node):
                return action
            # add new actions and new node to frontier
    ...
    while not frontier.isEmpty():
        ...
        if node not in reached:
            ...
            for nextNode, nextAction, nextCost in problem.getSuccessors(node):
                ...
                priority = cost + nextCost
                #hfunction 
                h_function = priority + heuristic(nextNode, problem)
                frontier.push((nextNode, newAction,priority),h_function)
    util.raiseNotDefined()
